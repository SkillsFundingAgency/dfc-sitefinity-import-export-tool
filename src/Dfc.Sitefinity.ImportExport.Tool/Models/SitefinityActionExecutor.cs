...
using Dfc.Sitefinity.ImportExport.Tool.Extensions;
...
{
    public static class SitefinityActionExecutor
    {
        ...
        public static async Task RunWorkflow(ISiteFinityHttpService service, ILogger logger, Workflow workflow)
        {
            foreach (var step in workflow.Steps)
            {
                ...
                switch (step.Action)
                {
                    ...
                    case Action.Extract:
                    {
                        var dir = Directory.CreateDirectory(Path.GetFullPath(step.Directory ?? Environment.CurrentDirectory));
                        await RunExtract(service, dir.FullName, step.ContentType);
                        logger.LogInformation($"Extracted {step.ContentType} to {dir.FullName}");
                        break;
                    }
        ...
        public static async Task RunWorkflowFromFile(ISiteFinityHttpService service, ILogger logger, string workflowFile)
        {
            var workflow = ReadWorkflow(workflowFile);

            logger.LogInformation($"Read workflow file {workflowFile}");

            await RunWorkflow(service, logger, workflow);
        }
    }
}